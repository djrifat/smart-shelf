...
while True:
    ...
    if total_frames % skip_frames == 0:
        ...
        for face in response:
            ...
            face_display = {
                ...
                'mood': current_mood,
                'origin': (start_x, top, width, height)
            } 
        ...
        for t in trackers:
            ...
            test_rect = utils.detector_utils.unpack_rect(frame, t, rgb, test_rect)
            (x1,x2,y1,y2) = test_rect    
    ...
    for face in response:
        ...
        #print("FACE RECTS: ", face_rect)
        #print("DISPLAYYYYYYY FIRST ", face_display)
        #print("FACEEEE: ", face)

        for i, k  in enumerate(face_display):
            print("DISPLAYYYYYYY ", k, face_display[k])
            text2 = "{0}: {1}".format(k, face_display[k])
            ...
            cv2.putText(frame, text2, 
                ...
                (x1+(y1-x1)+5, x2+5+20*i),cv2.FONT_HERSHEY_DUPLEX, 0.5, (0,255,0), 1, cv2.LINE_AA)
                #print
                #(centroid[0] - 10, centroid[1] - 10*i),cv2.FONT_HERSHEY_DUPLEX, 0.5, (0,255,0), 1, cv2.LINE_AA)
